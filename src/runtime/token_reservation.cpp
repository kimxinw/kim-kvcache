    ...
    std::uint32_t expected_committed_tokens)
{
    ...
    if (candidate.entries_.empty()
        || candidate.entries_.back().kind == PageKind::Extent
        || candidate.entries_.back().valid_tokens
            == kMicroPageTokenCapacity) {
        KvCacheError const allocation_error =
            allocateStagedMicroLocked(0, staged_target);
    ...
    KvTokenReservationId reservation_id)
{
    ...
    if (existing != nullptr) {
        ...
        if (committed_tail.valid_tokens == kMicroPageTokenCapacity) {
